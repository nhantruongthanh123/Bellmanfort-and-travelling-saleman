...
void travelling(int arr[][1000], char start, int n){
    if (n<=10){
    vector <int> a;
    int first = start - 'A';
    for (int i=0;i<n;i++){
    ...
    do {
        int distance = 0;
        for (int i=0;i<n-1;i++){
            if (i==0) distance+= arr[first][a[0]];
            else distance+= arr[a[i-1]][a[i]];
        }
    ...
    do {
        int distance = 0;
        for (int i=0;i<n-1;i++){
            if (i==0) distance+= arr[first][b[0]];
            else distance+= arr[b[i-1]][b[i]];
        }
...
void nearestneighbor(int arr[][1000], char start, int n){
    int nearest = INT_MAX;
    vector <int> ans;
    int road;
